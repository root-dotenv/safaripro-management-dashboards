...
import {
  ...
  FaDoorOpen,
  FaInfoCircle,
  FaCalendarTimes, // More appropriate for checked out
} from "react-icons/fa";
import { FiAlertTriangle } from "react-icons/fi";
import React from "react";

// Define the RoomDetails type (consistent with special-requests.tsx and checked-in.tsx)
...
// Define the Booking type, focusing on fields relevant for checked-out guests
interface Booking {
  id: string;
  full_name: string;
  email: string;
  ...
  checkin: string | null;
  checkout: string | null; // The key field to check for checked-out status
...
export default function CheckedOut() {
  ...
  const {
    ...
    queryKey: ["allBookingsForCheckedOut"], // Unique key for this specific query
    queryFn: async () => {
  ...
  // Filter bookings to get only those that have checked out
  const checkedOutBookings = React.useMemo(() => {
    return (
      bookingsData?.results.filter(
        (booking) => booking.checkout !== null && booking.checkout !== ""
      ) || []
    );
  }, [bookingsData]);

  // Fetch room details for each checked-out booking
  const {
    data: roomsData,
    ...
    queryKey: [
      "roomsForCheckedOut",
      checkedOutBookings.map((b) => b.property_id).join(","),
    ...
    queryFn: async () => {
      ...
      const uniquePropertyIds = Array.from(
        new Set(checkedOutBookings.map((booking) => booking.property_id))
      );
    ...
    enabled: checkedOutBookings.length > 0, // Only run the query if there are checked-out bookings
  ...
  if (bookingsError || roomsError) {
    return (
      <NoDataMessage
        ...
        title="Error Loading Checked-Out Guests!"
        description={
  ...
  if (checkedOutBookings.length === 0) {
    return (
      <NoDataMessage
        icon={FaCalendarTimes} // Using FaCalendarTimes for checked out
        title="No Guests Checked Out"
        description="There are currently no guests with a defined check-out date."
        type="info"
      />
    );
  }
  ...
  return (
    <div className="w-full h-full min-h-screen pb-[1rem] px-4 py-6">
      ...
      <p className="text-[#202020] text-[0.9375rem] font-medium mt-1 flex items-center mb-6">
        ...
        View all guests who have successfully checked out from your hotel.
      </p>
                      ...
                      <span
                        className={`px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          booking.payment_status === "Pending"
                            ? "bg-[#FEF9C3] text-[#F59E0B]" // Orange/Yellow
