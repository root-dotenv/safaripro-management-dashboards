// - - - safaripro_admin/src/pages/bookings/safaripro-bookings.tsx
...
export default function SafariproBookings() {
  ...
  const filteredBookings = useMemo(() => {
    ...
    // --- NEW: Filter by booking_type "Online" ---
    currentBookings = currentBookings.filter(
      (booking) => booking.booking_type === "Online"
    );
  ...
  const handleEdit = (bookingId: string) => {
    navigate(`/bookings/safaripro-bookings/edit/${bookingId}`); // Navigate to the new edit page
  };
  ...
  const exportBookingsToCsv = (bookingsToExport: Booking[]) => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute("download", `safaripro_online_bookings_report.csv`);
      ...
      <div className="w-full px-4 py-4">
        <div className="flex items-center gap-x-4">
          ...
          <h1 className="text-[1.375rem] text-[#202020] font-bold text-center">
            SafariPro Online Bookings Overview
          </h1>
        </div>
        <p className="text-[#202020] text-[0.9375rem] font-medium mt-1 flex items-center">
          ...
          View and manage all online bookings made through SafariPro for your
          hotel.
        </p>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {sortedBookings.map((booking, index) => (
                <tr key={booking.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-sm text-[#202020]">
                    <div className="flex items-center">
                      {/* Removed FaBuilding here as it's not present in AllRooms/HotelAmenities for similar context */}
                      {booking.microservice_item_name}
                    </div>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap text-sm text-[#202020]">
